...
export const impactSnapshotSchema = z.object({
  ...
  createdAt: z.number().int().min(1),
}) as unknown as z.ZodType<ImpactSnapshot>
...
export const deliverableLocalDomainSpec = defineDomain({
  ...
  tables: {
    ...
    save_keys: domainTable<string, SaveKeyEntry>(saveKeySchema),
    latest: domainTable<string, LatestVersionEntry>(latestVersionSchema),
    impact_snapshots: domainTable<string, ImpactSnapshot>(impactSnapshotSchema),
  },
})
