...
export function WorkbenchDrawer(props) {
    ...
    if (!open)
        return null;
    return (_jsxs("div", { ref: drawerRef, className: css.drawer, style: { width: `${width}px` }, role: "dialog", "aria-label": t('trigger'), children: [_jsx("div", { ref: resizeRef, className: css.resize, role: "separator", "aria-orientation": "vertical", "aria-label": t('resize'), onPointerDown: onResizeDown, onPointerMove: onResizeMove, onPointerUp: onResizeUp }), _jsxs("div", { className: css.head, children: [_jsx("span", { className: css.headTitle, children: t('trigger') }), _jsx("button", { type: "button", className: css.close, onClick: () => { actions.closeDrawer(); }, children: t('close') })] }), _jsx("div", { className: css.tabs, role: "tablist", children: ['tasks', 'taskList', 'recipeLibrary', 'inbox', 'clarifications', 'detail', 'create'].map(key => (_jsx("button", { type: "button", role: "tab", "aria-selected": tab === key, className: tab === key ? css.tabOn : css.tabOff, onClick: () => { selectTab(key); }, children: t(`tab.${key}`) }, key))) }), _jsxs("div", { className: css.body, children: [tab === 'tasks' && renderSlot('workbench.drawer.tasks', owner), tab === 'taskList' && renderSlot('workbench.drawer.taskList', owner), tab === 'recipeLibrary' && renderSlot('workbench.drawer.recipeLibrary', owner), tab === 'inbox' && renderSlot('workbench.drawer.inbox', {}), tab === 'clarifications' && renderSlot('workbench.drawer.clarifications', {}), tab === 'detail' && renderSlot('workbench.drawer.detail', { taskId: detailTaskId, openInbox: owner.openInbox }), tab === 'create' && renderSlot('workbench.drawer.create', owner)] })] }));
}
