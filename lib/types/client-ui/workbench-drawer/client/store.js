...
export function createWorkbenchStore() {
    return defineStore({
        init: () => ({ open: false, tab: 'tasks', detailTaskId: undefined, createRecipeId: undefined }),
        actions: {
            openDrawer: (d) => { d.open = true; },
            closeDrawer: (d) => { d.open = false; },
            toggleDrawer: (d) => { d.open = !d.open; },
            selectTab: (d, tab) => { d.tab = tab; },
            openDetail: (d, taskId) => { d.tab = 'detail'; d.detailTaskId = taskId; },
            setDetailTaskId: (d, taskId) => { d.detailTaskId = taskId; },
            openCreate: (d, recipeId) => { d.tab = 'create'; d.createRecipeId = recipeId; },
