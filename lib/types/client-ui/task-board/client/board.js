...
export class TaskBoardController {
    ...
    constructor(ctx) {
        this.ctx = ctx;
        this.store = createSnapshotStore({ status: 'loading', tasks: [], metrics: undefined, phaseProgress: new Map(), taskGates: new Map(), updatedAt: 0 });
    ...
    async refresh() {
        ...
        const entries = await Promise.all(tasks.map(async (task) => {
            if (task.currentRunId === undefined)
                return [task.taskId, { current: 0, total: 0 }, undefined];
            const runs = await this.ctx.remote.tasks.listPhaseRuns(String(task.currentRunId));
            if (!runs.ok)
                return [task.taskId, { current: 0, total: 0 }, undefined];
            ...
            return [task.taskId, progress, gate];
        }));
