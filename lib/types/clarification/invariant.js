...
function openDomain(ctx, fail) {
    ...
    if (domain === undefined)
        return fail(`${DOMAIN_NAME} changed while the clarification domain is not open`);
    return domain;
}
/**
...
const install = Object.assign((ctx, fail) => {
    ctx.on('domain/changed', (change) => {
        ...
        const requestExists = (requestId) => requests.get(requestId) !== undefined;
        const questionExists = (questionId) => questions.get(questionId) !== undefined;
        if (change.table === 'questions') {
            const question = change.value;
