...
import { taskFactKey } from "./spec.js";
...
function factKeyOf(table, value) {
    if (table === 'tasks')
        return taskFactKey('task/updated', String(value.taskId), Number(value.revision));
    if (table === 'task_runs')
        return taskFactKey('task-run/updated', String(value.runId), Number(value.revision));
    if (table === 'phase_runs')
        return taskFactKey('phase-run/updated', String(value.phaseRunId), Number(value.revision));
    if (table === 'submissions')
        return taskFactKey('submission/recorded', String(value.submissionId), 1);
    return undefined;
}
/**
