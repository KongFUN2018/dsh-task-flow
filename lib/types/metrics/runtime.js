 ...
 */
export function buildWorkbenchMetrics(tasks, items, versions, facts) {
    ...
    for (const fact of facts) {
        if (fact.kind !== PHASE_RUN_UPDATED || fact.occurredAt < windowStart)
            continue;
    ...
    const throughput = [...throughputByDay.entries()]
        .sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0))
        .map(([day, completedPhases]) => ({ day, completedPhases }));
    const counts = new Map();
    for (const fact of facts) {
        if (fact.kind !== GATE_CHECK_RECORDED)
            continue;
        const payload = fact.payload;
    ...
    const gatePassRate = { a: rate('A'), b: rate('B'), c: rate('C') };
 ...
 */
export function buildTaskMetrics(task, facts, budget) {
